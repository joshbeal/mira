use ark_bn254::{Fq, G2Affine as ArkG2Affine};
use ark_ec::{
...
use ark_std::One;
use halo2_proofs::halo2curves::{bn256, CurveAffine};
use mira::ff::{FromUniformBytes, PrimeField as FfPrimeField};
...
/// Converts a point of an arkworks short weierstrass curve to one on the corresponding halo2 curve.
/// This version works for points on G2, i.e., curves defined over quadratic extension fields.
/// Currently this method is specialized to the BN254 G2 group pending integration of the following
/// change in the halo2curves lib: https://github.com/privacy-scaling-explorations/halo2curves/pull/161
pub fn ark_to_halo2_g2_affine(p: ArkG2Affine) -> Result<bn256::G2Affine, GroupConversionError> {
    // Extract the coordinates from the arkworks point (assuming affine coordinates)
    let (x, y) = p.xy().ok_or(GroupConversionError::PointAtInfinity)?;

    let x_ff = x
        .to_base_prime_field_elements()
        .map(|element| ark_to_ff_field::<Fq, bn256::Fq, 64>(element))
        .collect::<Result<Vec<bn256::Fq>, _>>()
        .map_err(|_| GroupConversionError::BaseFieldMismatch)?;
    ...
    // Convert x and y coordinates from base field elements to extension field elements
    let hx = bn256::Fq2 {
        c0: x_ff[0],
        c1: x_ff[1],
    };
    ...
    // Construct the point on the curve
    Ok(bn256::G2Affine::from_xy(hx, hy).expect("point should be on curve"))
}

#[cfg(test)]
mod tests {
    use ark_bn254::{g1::Config as ArkBn254, Fq as ArkFq, Fr as ArkFr};
    use ark_ec::CurveGroup;
    use ark_std::{rand::RngCore, test_rng, UniformRand};
    use mira::halo2curves::{
        bn256::{Fr as FfFr, G1Affine as HaloBn254},
        group::Curve,
    };
    ...
    fn group_conversion_test() {
        group_conversion_test_helper::<ArkBn254, HaloBn254, 64>();
    }
}
